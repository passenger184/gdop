...
def toggle_active_status(modeladmin, request, queryset):
    for user in queryset:
        ...
        user.save()
    modeladmin.message_user(request, "Selected users' active status has been toggled.")
...
class AnnouncementAdmin(BaseAdmin):
    list_display = ["title", "sub_title", "updated_at", "created_by", "updated_by"]
...
class AboutUsAdmin(BaseAdmin):
    ...
    search_fields = ["title", "description_1", "description_2", "bullet_points"]
    list_filter = ["updated_at"]
...
class FooterContentAdmin(BaseAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("about_text", "logo_image")}),
        (
            "Contact Information",
            {
                "fields": (
